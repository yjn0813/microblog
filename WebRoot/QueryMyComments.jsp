  ...
  <head>
	...
	function checkDel(comments_id) {
		if (window.confirm("是否删除 ？")) {
			window.location.href = "deleteComments.do?comments_id=" + comments_id;
		}
	}
	</script>
	<style type="text/css">
	...
	<body
		...
		<font face="微软雅黑">
			<div class="blue">
				...
				<h3 class="header-title">我发出的评论</h3>
				</div>
	...
	<c:forEach var="commentsList" items="${requestScope.commentsList}">
				<table width="80%">
					<tr>
						<td width="6%"></td>
						<td width="10%">
						<strong><font size="4">${commentsList.follow_id}</font></strong>：
						</td>
						<td class="article_content">
							<font size="3">${commentsList.article_content}</font>
						</td>
					</tr>
					</table>
					<table width="80%">
				...
				<td width="6.5%"></td>
						<td rowspan="2" width="5%">
							<img class="profile-img" src="files/${commentsList.photo}"
					...
					<td width="3%"></td>
						<td class="profile-detail" width="5%">
							<font size="2" color="#0A8CD2"><strong>${commentsList.ID}
							</strong></font></td>
					<td width="3%"><font size="2">说：</font></td>
						<td class="comments_content" colspan="2">
							<font size="2">${commentsList.comments_content}</font>
						</td>
						<td><font size="2"><a href="javascript:checkDel('${commentsList.comments_id}')">删除</a></font></td>
					</tr>
